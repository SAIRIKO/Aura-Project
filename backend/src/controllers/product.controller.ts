...
export const productController = {
    ...
    async create(req: Request, res: Response) {
        try {
            const { name, description, category, activeIngredient, price, stock, imageUrl, pharmacyId } = req.body;
    ...
    async remove(req: Request, res: Response) {
        try {
            ...
            res.status(400).json({ error: 'Erro ao remover produto', details: error });
        }
    },
};
